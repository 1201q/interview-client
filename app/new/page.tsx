  ...
  return (
    <>
      <aside className="sidebar">
        <SidebarStepsHydrated initialStage="resumeText" />
      </aside>
      <main className="main">
...
const ResumeUploader = () => {
  ...
  const onChoose: React.ChangeEventHandler<HTMLInputElement> = (e) => {
    ...
    if (f.size > 5 * 1024 * 1024) return alert('최대 5MB까지 업로드 가능');
    setFile(f);
    setText('');
  };
  ...
  return (
    <>
      <div>
        ...
        <p className="title">이력서를 업로드하세요</p>
        <p className="desc">
        ...
        >
          ...
          >
            ...
            >
              ...
              <p className={styles.desc}>PDF · 최대 5MB</p>
              <input
                ref={inputRef}
                type="file"
                hidden
          ...
          >
            <p className="contentTitle">PDF 업로드</p>
      ...
      <div className="buttons">
        <Button
          ...
          onClick={() => setRequestStage('jobText')}
        />
      </div>
    </>
  );
...
const JobTextUploader = () => {
  ...
  return (
    <>
      <div>
        <p className="stepText">STEP 2 OF 4</p>
        <p className="title">채용공고를 업로드하세요</p>
        <p className="desc">
        ...
        >
          {/* 텍스트 입력 — 항상 마운트(포커스 유지), 접기/펼치기 */}
          <motion.section layout className="content">
            <div className={styles.textInput}>
              <textarea
                value={text}
                onChange={(e) => {
